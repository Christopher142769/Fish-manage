...
/* ========================= SIDEBAR COMPONENT ========================= */
function Sidebar({ companyName, onLogout, currentPage, onNavigate }) {
  const navItems = [
  ...
  return (
    <div 
      ...
      style={{ width: SIDEBAR_WIDTH, position: 'fixed', height: '100vh', top: 0, left: 0 }}
    >
      {/* Logo/Titre */}
                ...
                return (
                  <React.Fragment key={s._id}>
                    <tr className={rowClass}>
                      ...
                      <td className="fw-bold">{money(s.amount)}</td>
...
function ChartsPanel() {
  ...
  useEffect(() => {
    ...
    salesChart.current = new Chart(salesRef.current, {
      type: "bar", // Changement pour un bar chart pour un look plus moderne
      data: {
        labels: monthly.labels,
        datasets: [{ 
            label: "Ventes (montant XOF)", 
            data: monthly.amounts,
            backgroundColor: 'rgba(0, 123, 255, 0.7)', // Bleu primaire
    ...
    debtsChart.current = new Chart(debtsRef.current, {
      ...
      data: {
        ...
        datasets: [{ 
            ...
            data: monthly.balances,
            borderColor: 'rgb(220, 53, 69)', // Rouge danger
  ...
  return (
    <div className="row g-4 mb-4">
      <div className="col-lg-6">
        <div className="card border-0 shadow-sm rounded-4" style={{ height: 400 }}>
          <div className="card-body">
            <h5 className="fw-bold text-dark"><i className="bi bi-bar-chart-fill me-2 text-primary"></i>Volume des Ventes Mensuelles</h5>
            <hr className="my-2"/>
            {!chartReady ? <div className="text-muted small py-5 text-center">Chargement des graphiques…</div> : <canvas ref={salesRef} style={{ height: 320 }} />}
          </div>
        </div>
      </div>
      <div className="col-lg-6">
        <div className="card border-0 shadow-sm rounded-4" style={{ height: 400 }}>
          <div className="card-body">
            <h5 className="fw-bold text-dark"><i className="bi bi-file-earmark-bar-graph-fill me-2 text-danger"></i>Évolution des Dettes/Soldes</h5>
            <hr className="my-2"/>
            {!chartReady ? <div className="text-muted small py-5 text-center">Chargement des graphiques…</div> : <canvas ref={debtsRef} style={{ height: 320 }} />}
          </div>
        </div>
      </div>
    </div>
...
export default function App() {
  ...
  const [currentPage, setCurrentPage] = useState("dashboard"); // State pour la navigation
  ...
  return (
    <div className="d-flex">
      ...
      <main 
          ...
          style={{ marginLeft: SIDEBAR_WIDTH, background: "#f0f2f5", minHeight: "100vh" }}
      >

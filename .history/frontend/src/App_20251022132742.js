...
function Sidebar({ companyName, onLogout, currentPage, onNavigate }) {
  ...
  // Gestion du responsive de la sidebar (masquée sur très petits écrans)
  const isSmallScreen = window.innerWidth < 768;

  if (isSmallScreen) {
    // Si vous prévoyez une vraie navigation mobile (hamburger menu), 
    // il faudrait implémenter ici l'ouverture/fermeture d'un menu déroulant.
    // Pour l'instant, on se concentre sur le layout desktop/tablette.
  }

  return (
    <div 
      className="d-none d-md-flex flex-column flex-shrink-0 p-3 text-white bg-dark shadow-lg" 
      style={{ width: SIDEBAR_WIDTH, position: 'fixed', height: '100vh', top: 0, left: 0, zIndex: 1030 }}
    >
      {/* Logo/Titre */}
...
export default function App() {
  ...
  return (
    <div className="d-flex">
      ...
      {/* Contenu Principal - Utilisation de ms-md-auto et style pour responsive */}
      <main 
          className="flex-grow-1 ms-md-0" // ms-md-0 pour le responsive
          style={{ 
              marginLeft: window.innerWidth >= 768 ? SIDEBAR_WIDTH : 0, 
              background: "#f0f2f5", 
              minHeight: "100vh",
              transition: 'margin-left 0.3s ease' // Ajout d'une transition pour le style
          }}
      >

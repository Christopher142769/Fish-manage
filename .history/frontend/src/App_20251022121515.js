          ...
          <div className="card border-0 shadow rounded-4">
            <div className="card-body p-4">
              ...
              <p className="text-center text-muted">
                Tilapia & Pangasius • Historique • Livraisons • Dettes • Export • Graphiques
              </p>
      ...
      <div className="card-body">
        <div className="d-flex flex-wrap gap-2 align-items-center mb-2">
          ...
          <div className="ms-auto d-flex gap-2">
            ...
            <button className="btn btn-outline-success" onClick={exportExcel}>Exporter Excel</button>
          </div>
        </div>
                          ...
                          <div className="bg-light p-3 rounded-3">
                            {actionType === "deliver" ? (
                              <div className="d-flex align-items-end gap-2 flex-wrap">
                                ...
                                <button className="btn btn-primary" onClick={()=>submitAction(s)}>Valider la livraison</button>
                              ...
                              <div className="d-flex align-items-end gap-2 flex-wrap">
                                ...
                                <button className="btn btn-secondary" onClick={()=>submitAction(s)}>Valider le règlement</button>
                                <button className="btn btn-link" onClick={()=>{ setOpenRow(null); setActionType(""); }}>Annuler</button>
                              </div>
                            )}
                          </div>
...
function ChartsPanel() {
  ...
  const monthly = useMemo(() => {
    ...
    sales.forEach((s) => {
      ...
      const curr = map.get(k) || { amount: 0, balance: 0 };
    ...
    const balances = labels.map((k) => map.get(k).balance);
    return { labels, amounts, balances };
  }, [sales]);
  ...
  useEffect(() => {
    ...
    salesChart.current = new Chart(salesRef.current, {
      ...
      data: {
        ...
        datasets: [{ label: "Ventes (montant)", data: monthly.amounts }],
      },
    ...
    debtsChart.current = new Chart(debtsRef.current, {
      ...
      data: {
        labels: monthly.labels,
        datasets: [{ label: "Dettes (solde)", data: monthly.balances }],
      },
      options: { responsive: true, maintainAspectRatio: false },
    });
  }, [chartReady, monthly]);
  ...
  return (
    <div className="row g-3">
      <div className="col-lg-6">
        <div className="card border-0 shadow rounded-4" style={{ height: 320 }}>
          <div className="card-body">
            <h6>Ventes mensuelles</h6>
            {!chartReady ? <div className="text-muted small">Chargement des graphiques…</div> : <canvas ref={salesRef} style={{ height: 250 }} />}
          </div>
        </div>
      </div>
      <div className="col-lg-6">
        <div className="card border-0 shadow rounded-4" style={{ height: 320 }}>
          <div className="card-body">
            <h6>Dettes mensuelles</h6>
            {!chartReady ? <div className="text-muted small">Chargement des graphiques…</div> : <canvas ref={debtsRef} style={{ height: 250 }} />}
          </div>
        </div>
      </div>
    </div>
...
/* ========================= NOTIFICATIONS D’ÉCHÉANCE =========================
   - On considère une dette "en retard" si la vente a un solde > 0 ET que
     la date est plus ancienne que "thresholdDays" (paramétrable).
    ...
    <div className="card border-0 shadow rounded-4 mb-3">
      <div className="card-body">
        <div className="d-flex align-items-center mb-2">
          <h5 className="m-0">Notifications d’échéance</h5>
          <div className="ms-auto d-flex gap-2">
            <div className="input-group" style={{ width: 240 }}>
              <span className="input-group-text">Retard ≥</span>
            ...
            {perm !== "granted" ? (
              <button className="btn btn-outline-primary" onClick={askPerm}>Activer notifications</button>
            ) : (
              <button className="btn btn-outline-secondary" onClick={notifyNow}>Tester une alerte</button>
            )}
          </div>
        </div>
        ...
        <div className="table-responsive">
          <table className="table align-middle">
            <thead>
            <tr><th>Client</th><th>Depuis</th><th>Jours</th><th>Solde</th></tr>
            </thead>
            <tbody>
            {overdue.length === 0 && <tr><td colSpan="4" className="text-center py-3">Aucune dette en retard</td></tr>}
            {overdue.map(o => (
              <tr key={o.id}>
                <td className="fw-semibold">{o.client}</td>
                <td>{o.date.toISOString().slice(0,10)}</td>
                <td className="text-danger fw-bold">{o.days}</td>
                <td className="text-danger fw-bold">{money(o.balance)}</td>
              </tr>
            ))}
            </tbody>
          </table>
        </div>
        <div className="text-muted small">Astuce : garde l’onglet ouvert pour recevoir les alertes. (Pour des notifications serveur / e-mail, il faudra une petite API en plus.)</div>
      </div>
    </div>
  );
}
...
export default function App() {
  // Afficher la page d'auth tant que pas connecté

...
function SaleForm({ onSaved }) {
  ...
  const save = async (e) => {
    ...
    try {
      ...
      // 2. Validation du paiement : PAIEMENT N'EST PLUS PLAFONNÉ
      // Suppression de : if (p > totalAmount) { ... }
      // --------------------------------------------------
      ...
      <div className="card-body p-4">
        <form onSubmit={save} className="row g-3">
          ...
          <div className="col-12 d-flex justify-content-between flex-wrap pt-3 mt-3 border-top">
            ...
            {/* Affichage du solde réel (balance) : Si négatif, c'est un crédit */}
  ...
  const submitAction = async (sale) => {
    try {
      if (actionType === "deliver") {
        ...
        // L'ancienne validation qui plafonnait le paiement est supprimée.
        // --------------------------------------------------
          ...
          <table className="table align-middle table-hover border-light">
            ...
            <tbody>
              ...
              {sales.map((s) => {
                ...
                // Solde affiché: si négatif, c'est un crédit
                const balance = s.balance || 0;
                // Montant à payer est toujours Math.max(0, balance) pour le bouton "Max"
                const remainingToPay = Math.max(0, balance); 
                
                // Classe de la ligne : Danger si dette client (balance > 0), Succès si crédit client (balance < 0)
                let rowClass = "";
                    ...
                    <tr className={rowClass}>
                      ...
                      <td>
                        <div className="d-flex flex-wrap gap-2">
                          ...
                          {/* Le bouton Régler est toujours possible, même si la balance est négative, car on peut ajouter au crédit */}
                          {/* On désactive le bouton si la vente est soldée mais qu'il n'y a plus de crédit non plus (balance=0) */}
                          <button 
                          ...
                          {/* Bouton Solder visible uniquement s'il y a une dette (balance > 0) */}
                          {balance > 0 && (
                            ...
                            {actionType === "deliver" ? (
                              ...
                              <div className="d-flex align-items-center gap-3 flex-wrap">
                                ...
                                <div className="input-group" style={{ maxWidth: 350 }}>
                                  ...
                                  <input type="number" min="0" step="0.01" className="form-control" value={actionValue} onChange={(e) => setActionValue(e.target.value)} />
                                  {/* Le bouton Max met le montant restant, mais l'utilisateur peut payer plus */}
                                  {remainingToPay > 0 && ( 
...
function ChartsPanel() {
  ...
  const data = useMemo(() => {
    ...
    sales.forEach((s) => {
      ...
      // Pour les graphiques, on veut la dette client (solde > 0). On ignore le crédit (balance < 0).
...
function SummaryCards() {
  ...
  // Le solde total peut maintenant être négatif s'il y a plus de crédits que de dettes
  ...
  const renderPage = () => {
    switch (currentPage) {
      ...
      case "debts":
        return (
          <>
            {/* 🚨 MODIFICATION : Ajout de CreditsBoard */}
            <div className="row g-4 mb-4">
        ...
        return (
          <>
            ...
            <div className="row g-4 mt-1">
              {/* 🚨 MODIFICATION : Mise en page pour inclure CreditsBoard */}
              <div className="col-lg-6"><DebtsBoard /></div>
              <div className="col-lg-6"><CreditsBoard /></div>
            </div>

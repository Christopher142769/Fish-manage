...
function Sidebar({ companyName, currentPage, onNavigate, onLogout, open, setOpen, isMdUp }) {
  const navItems = [
    ...
    { id: "client-report", icon: "bi-file-earmark-bar-graph-fill", label: "Bilan Client" }, // 🚨 NOUVEL ÉLÉMENT
  ...
  const submitAction = async (sale) => {
    try {
      if (actionType === "deliver") {
        ...
        // Note: Le backend gère la compensation, ici on met à jour les données du tableau
        // pour refléter l'état de la vente principale et potentiellement des autres (via reload)
        setSales((prev) => prev.map((s) => (s._id === sale._id ? data : s)));
        // Déclenche un rechargement complet pour s'assurer que les autres ventes compensées sont mises à jour
        window.dispatchEvent(new Event("reload-sales")); 
          ...
          setSales((prev) => prev.map((s) => (s._id === sale._id ? data : s)));
          // Déclenche un rechargement complet si la balance est remontée au-dessus de 0
          window.dispatchEvent(new Event("reload-sales")); 
...
export default function App() {
  ...
  const renderPage = () => {
    switch (currentPage) {
      ...
      case "client-report": // 🚨 NOUVELLE PAGE

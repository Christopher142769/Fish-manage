...
function Sidebar({ companyName, currentPage, onNavigate, onLogout, open, setOpen, isMdUp }) {
  ...
  return (
    <>
      ...
      <aside
        className={`bg-dark text-white shadow-lg d-flex flex-column p-3 position-fixed top-0 ${
          isMdUp ? "start-0" : open ? "start-0" : "start-0 translate-middle-x"
        }`}
        style={{ width: SIDEBAR_WIDTH, height: "100vh", zIndex: 1030, transition: "transform .25s ease" }}
      >
        <button
          type="button"

...
function Sidebar({ companyName, onLogout, currentPage, onNavigate, isOpen, onClose }) {
  ...
  return (
    <>
      ...
      {isOpen && (
        <div 
          ...
          style={{ 
            ...
            bottom: 0, 
            left: 0, 
            zIndex: 1040, // Au-dessus du contenu mais sous la sidebar
      ...
      <div 
        className={`flex-column flex-shrink-0 p-3 text-white bg-dark shadow-lg sidebar-custom ${isOpen ? 'open' : 'closed'}`} 
        style={{ 
          width: SIDEBAR_WIDTH, 
          position: 'fixed', 
          height: '100vh', 
          top: 0, 
          left: 0, 
          zIndex: 1050, // Z-index élevé pour être au-dessus du contenu et de l'overlay
          transition: 'transform 0.3s ease',
          transform: isOpen ? 'translateX(0)' : 'translateX(-100%)', // Déplacement pour l'ouverture/fermeture
        }}
      >
        {/* Logo/Titre */}
        <a href="#" className="d-flex align-items-center mb-3 mb-md-0 me-md-auto text-white text-decoration-none">
          ...
          <button className="btn btn-dark d-md-none ms-auto p-0" onClick={onClose}>
          ...
          {navItems.map(item => (
            <li className="nav-item" key={item.id}>
              <button 
                  ...
                  onClick={() => { onNavigate(item.id); onClose(); /* Fermer après navigation sur mobile */ }}
              >
       ...
       {/* Style pour la transition de la sidebar (Ajouté ici pour la simplicité) */}
       <style>{`
          /* Sur desktop/tablet, la sidebar est toujours visible */
          @media (min-width: 768px) {
            .sidebar-custom {
              transform: translateX(0) !important; /* Force l'affichage */
            }
          }
          /* Sur mobile, on gère la transition */
...
function AuthView({ onAuth }) {
  ...
  return (
    <div className="d-flex align-items-center justify-content-center" style={{ minHeight: "100vh", background: "#f8f9fa" }}>
      <div className="col-12 col-sm-8 col-md-6 col-lg-4 p-3"> {/* Utilisation de col-12 sur mobile */}
    ...
    <div className="card border-0 shadow rounded-4 mb-4 bg-white">
      ...
      <div className="card-body p-4">
        <form onSubmit={save} className="row g-3">
          ...
          <div className="col-12 col-md-6"> {/* Responsive */}
          ...
          <div className="col-12 d-flex justify-content-between flex-wrap pt-3 mt-3 border-top gap-2"> {/* Ajout de gap-2 pour mobile */}
  ...
  return (
    <div className="card border-0 shadow rounded-4 bg-white">
      <div className="card-body p-4">
        ...
        {/* En-tête avec fond léger pour un look pro (Responsive avec flex-wrap) */}
        <div className="d-flex flex-wrap gap-3 align-items-center mb-4 p-3 bg-light rounded-3 border">
          ...
          <div className="ms-auto d-flex flex-wrap gap-2"> {/* Ajout de flex-wrap pour alignement sur mobile */}
             <div className="input-group" style={{minWidth: 150}}> {/* minWidth pour éviter un trop petit champ */}
          ...
          <table className="table align-middle table-hover border-light">
            <thead>
              <tr className="table-dark">
                <th>Date</th><th>Client</th><th className="d-none d-sm-table-cell">Poisson</th> {/* Caché sur petit écran */}
                <th>Qté (Kg)</th><th className="d-none d-md-table-cell">Livré (Kg)</th><th>Reste</th>
                <th className="d-none d-lg-table-cell">Prix</th><th>Montant</th><th className="d-none d-md-table-cell">Payé</th><th>Solde</th>
                <th className="d-none d-sm-table-cell">Statut</th><th style={{width:220}}>Actions</th>
              </tr>
            </thead>
            <tbody>
                ...
                return (
                  <React.Fragment key={s._id}>
                    <tr className={rowClass}>
                      ...
                      <td className="d-none d-sm-table-cell"><BadgeFish type={s.fishType} /></td> {/* Caché sur petit écran */}
                      <td>{s.quantity}</td>
                      <td className="d-none d-md-table-cell">{s.delivered || 0}</td>
                      <td className={remainingToDeliver > 0 ? "text-warning fw-bold" : ""}>{remainingToDeliver}</td>
                      <td className="d-none d-lg-table-cell">{money(s.unitPrice)}</td>
                      <td>{money(s.amount)}</td>
                      <td className="d-none d-md-table-cell">{money(s.payment)}</td>
                      <td className={s.balance > 0 ? "text-danger fw-bold" : ""}>{money(s.balance)}</td>
                      <td className="d-none d-sm-table-cell"> {/* Caché sur petit écran */}
                        {s.settled ? (
                      ...
                      <td>
                        <div className="d-flex flex-wrap gap-2 justify-content-center"> {/* Centrage pour mobile */}
                          ...
                          {!s.settled && (
                            <>
                              ...
                              <button className="btn btn-sm btn-outline-success rounded-circle d-none d-lg-inline" title="Solder toute la dette"
                                onClick={() => settleAll(s._id)}>
                                <i className="bi bi-currency-dollar"></i>
                              </button>
                            </>
                          )}
                           {/* Bouton Solder sur mobile si pas réglé et pas visible en lg */}
                           {!s.settled && <button className="btn btn-sm btn-outline-success rounded-pill d-lg-none" onClick={() => settleAll(s._id)}>Solder</button>}
                        </div>
                      </td>
                    </tr>
                      ...
                      <tr>
                        <td colSpan="12">
                          <div className="bg-light p-3 rounded-3 border border-secondary">
                            {actionType === "deliver" ? (
                              <div className="d-flex align-items-center gap-3 flex-wrap"> {/* flex-wrap pour mobile */}
...
function ChartsPanel() {
  ...
  // Utilisation de col-12 sur mobile, col-lg-6 sur tablette/petit desktop, col-xl-4 sur grand desktop
  return (
    <div className="row g-4 mb-4">
      <div className="col-12 col-lg-6 col-xl-4">
        <div className="card border-0 shadow-sm rounded-4 h-100">
          <div className="card-body">
            ...
            {/* Hauteur fixe pour éviter le décalage, mais Chart.js est responsive */}
      ...
      <div className="col-12 col-lg-6 col-xl-4">
       ...
       <div className="col-12 col-lg-6 col-xl-4"> {/* Un seul graphique sur une ligne en grand écran */}
        <div className="card border-0 shadow-sm rounded-4 h-100">
          <div className="card-body">
...
function DueNotificationsPanel() {
  ...
  return (
    <div className="card border-0 shadow rounded-4 mb-4 bg-white">
      <div className="card-body p-4">
        <div className="d-flex flex-wrap align-items-center mb-4 pb-2 border-bottom gap-3"> {/* flex-wrap pour mobile */}
        ...
        <div className="table-responsive">
          <table className="table align-middle table-striped">
            ...
            <tr><th>Client</th><th className="d-none d-sm-table-cell">Date Opération</th><th>Jours de Retard</th><th>Solde Dû</th></tr>
            </thead>
            <tbody>
            ...
            {overdue.map(o => (
              <tr key={o.id} className="table-warning-subtle">
                ...
                <td className="d-none d-sm-table-cell">{o.date.toISOString().slice(0,10)}</td>
...
function SummaryCards() {
  ...
  const Card = ({ title, amount, iconClass, cardClass }) => (
    <div className="col-12 col-md-4"> {/* Responsive: 100% sur mobile, 1/3 sur desktop */}
  ...
  return (
    <div className="row g-4 mb-5">
      ...
      />
      <div className="col-12 col-md-6"> {/* Responsive: 100% sur mobile, 1/2 sur desktop */}
...
function DebtsBoard() {
  ...
  return (
    <div className="card border-0 shadow rounded-4 bg-white">
      <div className="card-body p-4">
        <div className="d-flex flex-wrap align-items-center mb-4 pb-2 border-bottom gap-2"> {/* flex-wrap pour mobile */}
...
export default function App() {
  ...
  // Hook pour fermer la sidebar automatiquement sur desktop (pour l'initialisation)
  useEffect(() => {
    const handleResize = () => {
      if (window.innerWidth >= 768) {
        setIsSidebarOpen(true); // Toujours ouverte sur desktop
      } else {
        setIsSidebarOpen(false); // Fermée par défaut sur mobile
      }
    };
              ...
              return (
                  <>
                      ...
                      <div className="row g-4 mt-1">
                        <div className="col-12 col-lg-4"><DebtsBoard /></div> {/* Responsive: 100% sur mobile, 1/3 sur desktop */}
                        <div className="col-12 col-lg-8"><ReloadableSalesTable /></div> {/* Responsive: 100% sur mobile, 2/3 sur desktop */}
                      </div>
                  </>
              );
      }
  ...
  // Calcul de la marge gauche pour pousser le contenu hors de la sidebar sur desktop/tablette
  const mainContentMarginLeft = window.innerWidth >= 768 ? SIDEBAR_WIDTH : 0;
  ...
  return (
    <div className="d-flex">
      ...
      <Sidebar 
          ...
          isOpen={isSidebarOpen} // État passé à la sidebar
          onClose={() => setIsSidebarOpen(false)} // Callback pour la fermer
      />
      ...
      <main 
          ...
          style={{ 
              marginLeft: mainContentMarginLeft, // Marge dynamique pour desktop
              background: "#f0f2f5", 
              minHeight: "100vh",
      ...
      >
        <div className="container-fluid py-4">
          ...
          {/* Header de la page (avec bouton hamburger sur mobile) */}
          <div className="d-flex align-items-center mb-4 pb-2 border-bottom border-secondary-subtle">
             {/* Bouton Hamburger sur mobile (d-md-none) */}
             <button className="btn btn-outline-dark me-2 d-md-none" onClick={() => setIsSidebarOpen(true)}>
                <i className="bi bi-list fs-4"></i>
             </button>
             <h1 className="h2 m-0 text-dark fw-light fs-3">{getPageTitle(currentPage)}</h1> {/* fs-3 pour mieux s'adapter sur mobile */}

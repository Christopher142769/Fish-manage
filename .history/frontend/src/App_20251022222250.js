...
// 🚨 NOUVELLE FONCTION : Déclenche la compensation globale pour un client
/** Déclenche la compensation globale pour un client (pour re-matcher dettes/crédits) */
async function triggerClientCompensation(clientName) {
    ...
    try {
        ...
        window.dispatchEvent(new Event("reload-sales")); // Recharger tout le dashboard
    } catch(e) {
...
function CreditUseModal({ sale, onClose, onRefundSuccess, onNewSaleSuccess }) {
    ...
    const [newSaleFormData, setNewSaleFormData] = useState({
        ...
        delivered: '',
        unitPrice: '',
        payment: 0, // Laisser l'utilisateur entrer un paiement supplémentaire si nécessaire
        observation: '',
    });
    ...
    // 🚨 LOGIQUE SIMPLIFIÉE : On crée une nouvelle vente (dette) et le BACKEND gère la compensation
    const handleNewSale = async () => {
        ...
        try {
            ...
            // 1. Créer la nouvelle vente comme une dette ou une vente normale.
            // Le paiement par CREDIT se fera automatiquement par la compensation globale sur le serveur
            // en utilisant le crédit existant pour solder cette nouvelle dette.
            const res = await apiFetch("/api/sales", {
                method: "POST",
                body: JSON.stringify({
                    date: new Date().toISOString().slice(0, 10),
                    clientName: sale.clientName, // Important: utilise le client de la ligne de crédit
                    fishType: newSaleFormData.fishType,
                    quantity: q,
                    delivered: d,
                    unitPrice: u,
                    payment: newSaleFormData.payment || 0, // Paiement initial (peut être 0)
                    observation: `Vente potentiellement payée par CREDIT client (Utilisation automatique si crédit disponible). ${obs}`,
                }),
            });
            const data = await res.json();
            if (!res.ok) throw new Error(data.error || "Erreur lors de la création de la vente");

            alert(`Nouvelle vente de ${money(totalSaleAmount)} créée. Le solde/crédit client a été ajusté par compensation.`);
            onNewSaleSuccess();
                        ...
                        {useType === 'new-sale' && (
                            <form onSubmit={(e) => { e.preventDefault(); handleNewSale(); }}>
                                <div className="alert alert-info small">
                                    Enregistrez une nouvelle vente pour ce client. La **compensation automatique** utilisera le crédit disponible pour solder (ou réduire) cette nouvelle dette.
                                </div>
                                ...
                                <div className="d-grid mt-4">
                                    <button type="submit" className="btn btn-secondary btn-lg" disabled={loading}>
                                        ...
                                        {loading ? "Traitement..." : "Confirmer Vente et Compensation"}
                                    </button>
                                </div>
                            </form>
                        )}
  ...
  const submitAction = async (sale) => {
    try {
      if (actionType === "deliver") {
        ...
        setSales((prev) => prev.map((s) => (s._id === sale._id ? data : s)));
        window.dispatchEvent(new Event("reload-sales")); // Déclenche le rechargement pour visualiser la compensation
...
function SummaryCards() {
  ...
  useEffect(() => { (async () => {
    // 🚨 AJOUT DE L'ÉCOUTE DE L'ÉVÉNEMENT (POUR RECHARGER LES CARTES APRÈS COMPENSATION)
...
function DebtsBoard() {
  ...
  }, []); // Dépendances vides pour ne pas recréer la fonction à chaque render
  ...
  useEffect(() => { 
    ...
    window.addEventListener("reload-sales", handler);
    return () => window.removeEventListener("reload-sales", handler);
  }, [loadDebts]); // useEffect s'exécute à chaque changement de loadDebts
...
function CreditsBoard() {
  ...
  useEffect(() => { 
    ...
    window.addEventListener("reload-sales", handler);
    return () => window.removeEventListener("reload-sales", handler);
  }, [loadCredits]); // useEffect s'exécute à chaque changement de loadCredits

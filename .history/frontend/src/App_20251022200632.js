...
function CreditUseModal({ sale, onClose, onRefundSuccess, onNewSaleSuccess }) {
    ...
    const handleNewSale = async () => {
        setLoading(true);
        try {
            const q = Number(newSaleData.quantity || 0);
            const u = Number(newSaleData.unitPrice || 0);
            ...
            // Créer la nouvelle vente avec le paiement intégral par crédit
            const res = await apiFetch("/api/sales", {
                method: "POST",
                body: JSON.stringify({
                    date: new Date().toISOString().slice(0, 10),
                    clientName: sale.clientName,
                    fishType: newSaleData.fishType,
                    quantity: q,
                    delivered: q, // Supposons la livraison complète dans ce scénario
                    unitPrice: u,
                    payment: totalSaleAmount, // Le paiement utilise exactement le crédit
                    observation: `Paiement effectué par utilisation du crédit client (Vente ID: ${sale._id}).`,
                }),
            });
            const data = await res.json();
            if (!res.ok) throw new Error(data.error || "Erreur lors de la création de la vente");

            // Maintenant, rembourser la première vente du montant utilisé
            ...
            <div className="modal-dialog modal-dialog-centered">
                <div className="modal-content">
                    <div className="modal-header bg-success text-white">
                        <h5 className="modal-title">Utilisation du Crédit Client</h5>
                        <button type="button" className="btn-close btn-close-white" onClick={onClose}></button>
                    </div>
                    <div className="modal-body">
                        <div className="alert alert-success text-center">
                            Crédit disponible pour {sale.clientName} : 
                            <br />
                            <strong className="fs-4">{money(creditAvailable)}</strong>
                        </div>

                        <div className="d-flex justify-content-center mb-4">
                            <button 
                                className={`btn btn-lg me-2 ${useType === 'refund' ? 'btn-secondary' : 'btn-outline-secondary'}`}
                                onClick={() => setUseType('refund')}
                            >
                                <i className="bi bi-wallet2 me-2"></i> Remboursement Espèces
                            </button>
                            <button 
                                className={`btn btn-lg ${useType === 'new-sale' ? 'btn-secondary' : 'btn-outline-secondary'}`}
                        ...
                        {useType === 'new-sale' && (
                            <form onSubmit={(e) => { e.preventDefault(); handleNewSale(); }}>
                                <div className="alert alert-info small">
                                    Création d'une nouvelle vente de {sale.clientName}. Cette vente sera automatiquement payée par le crédit existant, et le solde de la vente initiale sera réduit.
                                </div>
                                
                                <div className="row g-3 mb-3">
                                    <div className="col-4">
                                        <label className="form-label small fw-semibold">Poisson</label>
                                        <select className="form-select" value={newSaleData.fishType} onChange={(e) => setNewSaleData(p => ({...p, fishType: e.target.value}))}>
                                            <option value="tilapia">Tilapia</option>
                                            <option value="pangasius">Pangasius</option>
                                        </select>
                                    </div>
                                    <div className="col-4">
                                        <label className="form-label small fw-semibold">Qté (Kg)</label>
                                        <input type="number" step="0.01" className="form-control" value={newSaleData.quantity} onChange={(e) => setNewSaleData(p => ({...p, quantity: e.target.value}))} required />
                                    </div>
                                    <div className="col-4">
                                        <label className="form-label small fw-semibold">Prix Unitaire</label>
                                        <input type="number" step="0.01" className="form-control" value={newSaleData.unitPrice} onChange={(e) => setNewSaleData(p => ({...p, unitPrice: e.target.value}))} required />
                                    </div>
                                </div>
                                ...
                                <div className="alert alert-warning small text-center">
                                    ...
                                    <strong className="fs-5 ms-2">
                                        {money(Number(newSaleData.quantity || 0) * Number(newSaleData.unitPrice || 0))}
                                    </strong>
                                    <br />

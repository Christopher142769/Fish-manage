  ...
  if (loading || !sum) {
      ...
      return (
        <div className="row g-4 mb-5">
            ...
            <CardLoading title="Solde/Encours" iconClass="bi-currency-exchange text-danger" cardClass="bg-danger text-white bg-opacity-75" />
...
/** =====================================
 * DEBTS BOARD 
 * (Utilisé tel quel pour la page Dettes, rendu conditionnel pour le Dashboard)
 * ===================================== */
...
function SalesBalancePage() {
  ...
  useEffect(() => {
    const loadSummary = async () => {
      setLoading(true);
      try {
        // isGlobal=true pour ignorer les filtres client/date et obtenir le total
...
function DashboardPage() {
    ...
    const hasFilter = !!selectedClient || !!startDate || !!endDate;
    ...
    const loadData = async (client, start, end) => {
        ...
        try {
            // 1. Charger les données de résumé/totaux
            ...
            // 2. Charger les données de ventes pour les graphiques et alertes
            ...
            setLoading(false);
            window.dispatchEvent(new Event("reload-sales")); // Pour rafraîchir les boards si nécessaire
        }
    };
    
    // Rechargement des données à chaque changement de filtre
    useEffect(() => {
        // Condition : Charger UNIQUEMENT si un filtre est actif (client OU date)
        if (hasFilter) {
            loadData(selectedClient, startDate, endDate);
        } else {
            // Pas de filtre sélectionné -> tout à zéro/vide (sans recharger)
            setSummaryData(null);
            setSalesData([]);
            // Le ReloadableSalesTableWrapper gèrera son propre état vide/initial.
        }
    ...
    return (
        <>
            ...
            <div className="row g-4 mt-1">
              <div className="col-lg-6">
                {/* Ces boards sont laissés à leur comportement par défaut (GLOBAL) mais affichent 0 si loading/initial */}
  ...
  const renderPage = () => {
    switch (currentPage) {
      ...
      case "charts":
        return (
          <>
            <ChartsPanel sales={[]} loading={false} />
            <SummaryCards sum={null} loading={false} />
          </>
        );
      case "dashboard":
      default:

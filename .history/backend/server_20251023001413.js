...
app.get('/api/summary', auth, async (req,res)=>{
  try{
    const { clientName, startDate, endDate, isGlobal } = req.query; // isGlobal pour la nouvelle page Bilan
    ...
    // Si isGlobal est présent, on ne filtre pas par client/date pour les dettes/crédits globaux.
    ...
    // Dettes et crédits globaux (Basé sur le filtre q SI isGlobal n'est PAS vrai. Si isGlobal=true, on prend tout.)
    const q_debts_credits = isGlobal === 'true' ? { owner: new mongoose.Types.ObjectId(req.user.uid) } : { ...q, clientName: q.clientName || undefined };

    // Les cartes Dettes/Crédits doivent montrer le montant ACTUEL, donc on ne filtre PAS par date,
    // MAIS on filtre par client si `clientName` est fourni.
    ...
    const [totalDebtResult] = await Sale.aggregate([
        { $match: { 
            ...(isGlobal === 'true' ? { owner: new mongoose.Types.ObjectId(req.user.uid) } : q_current_balance), 
            balance: { $gt: 0 } 
        } },
        { $group: { _id: null, totalDebt: { $sum: "$balance" } } },
        { $project: { totalDebt: 1, _id: 0 } }
...
// car la page "Dettes Clients" doit afficher les tops clients sur l'ensemble de l'historique.

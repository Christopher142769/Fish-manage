...
saleSchema.pre('validate', function(next){
  // Recalcul des champs dérivés avant chaque sauvegarde (création ou édition)
  this.delivered = Math.max(0, this.delivered || 0); // Assurer que delivered n'est pas négatif
  
  // Assurer que la livraison ne dépasse pas la quantité
...
// NOUVEAU MODÈLE: Journal des Actions
const actionLogSchema = new mongoose.Schema({
    owner: { type: mongoose.Schema.Types.ObjectId, ref: 'User', index: true, required: true },
    companyName: { type: String, required: true }, // Dénormalisé pour l'affichage
    actionType: { type: String, enum: ['edit', 'delete'], required: true },
    motif: { type: String, required: true, trim: true },
    saleId: { type: mongoose.Schema.Types.ObjectId, required: true }, // Garde l'ID original
    saleData: { type: Object, required: true } // Snapshot de la vente AVANT l'action
}, { timestamps: true }); // createdAt sera la date de l'action

const User = mongoose.model('User', userSchema);
const Sale = mongoose.model('Sale', saleSchema);
const ActionLog = mongoose.model('ActionLog', actionLogSchema); // NOUVEAU
...
/* ---------------- NOUVELLES ROUTES DE LOGS ---------------- */

// NOUVEAU: Récupérer tous les journaux d'actions
app.get('/api/action-logs', auth, async (req, res) => {
    try {
        const logs = await ActionLog.find({ owner: req.user.uid })
            .sort({ createdAt: -1 });
        res.json(logs);
    } catch(e) {
...
/* ---------------- Routes VENTES (MODIFIÉES) ---------------- */

// Créer une vente (inchangé, compensation auto retirée)
...
app.put('/api/sales/:id', auth, async (req, res) => {
    ...
    try {
        ...
        if (!motif || motif.trim() === '') {
            throw new Error("Le motif de l'édition est obligatoire.");
        }
        ...
        // 1. Enregistrer le log d'action AVANT de modifier
        await ActionLog.create([{
            ...
            motif: motif,
            saleId: sale._id,
            saleData: sale.toObject() // Snapshot avant modification
        }], { session });
        ...
        // 3. Valider (le pre-hook va recalculer amount, balance, settled)
        await sale.validate();
...
app.delete('/api/sales/:id', auth, async (req, res) => {
    ...
    try {
        ...
        // 1. Enregistrer le log d'action AVANT de supprimer
        await ActionLog.create([{
            owner: req.user.uid,
            companyName: req.user.companyName,
            actionType: 'delete',
            motif: motif,
            saleId: sale._id,
            saleData: sale.toObject() // Snapshot avant suppression
        }], { session });
...
// Lister ventes (inchangé, filtre par date)
...
// Rembourser une partie du crédit client (inchangé)
...
/* ---------------- Compensation MANUELLE (inchangée) ---------------- */
...
// Route de résumé général/filtré
...
// Routes /api/dashboard/debts et /api/dashboard/credits (inchangées)
